import { SceneUnionReq } from "@/app/api/union/scene.union.api";
import { useConfirm } from "@/common/ui/confirm.ui";
import { FC, useCallback, useEffect, useState } from "react";
import { useUpsertSceneUnion } from "./hooks";
import { periodApi } from "@/app/api/table/universe/period.api";
import UIFormControl from "@/common/ui/form-control.ui";
import Event_DatetimeInput from "../event/datetime-input.component";
import { Tables, TablesInsert } from "@supabase/database.type";
import Period_Editor from "../period/editor.component";

const Scene_Editor: FC<{ initReq: SceneUnionReq; onSave: () => void; onCancel: () => void }> = ({
  initReq,
  onSave,
  onCancel,
}) => {
  const { confirm } = useConfirm();
  const [req, setReq] = useState<SceneUnionReq>(initReq);
  const upsert = useUpsertSceneUnion();
